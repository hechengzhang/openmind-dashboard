    ...
    <div className="bg-white pt-[8px] rounded-[20px]">
      <Scrollbars style={{ height: 475 }}>
        <div className="min-w-[1000px]">
          <div className="grid grid-cols-4 border-b-[1px] border-[#EAECF0] text-[14px] font-[500] leading-[18px] text-secondary">
            <div className="px-[24px] py-[12px] w-[335px]">Device ({devicesArray.length})</div>
            <div className="px-[24px] py-[12px]">Status</div>
            <div className="px-[24px] py-[12px]">Duration of running</div>
            <div className="px-[24px] py-[12px] text-right">Credits Used</div>
          </div>
          {devicesArray.map((item, index) => {
            const { device: { image, name, id }, status, DurationOfRunning, points } = item
            return (
              <div key={index} className="grid grid-cols-4 text-primary">
                ...
                <div className="flex items-center px-[24px] py-[16px] text-[14px] leading-[20px]">
                  {DurationOfRunning === 0 ? '-' : DurationOfRunning + ' day'}
                </div>
